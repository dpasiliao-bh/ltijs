...
class Auth {
  ...
  static async generatePlatformKeyPair(
    ...
    while (
      await Database.Get(false, "publickey", {
        kid: kid,
      })
    ) {
  ...
  static async validateToken(
    ...
    } else if (validationParameters.iss !== decoded.payload.iss)
  ...
  static async validateNonce(token, Database) {
    ...
    if (
      await Database.Get(false, "nonce", {
        nonce: token.nonce,
      })
    )
  ...
  static async getAccessToken(scopes, platform, ENCRYPTIONKEY, Database) {
    ...
    const access = await got
      .post(await platform.platformAccessTokenEndpoint(), {
        form: message,
      })
      .json();
